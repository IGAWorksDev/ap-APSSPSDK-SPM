//
//  AdMobVideoMixAdapter.swift
//  MediationAdMob
//
//  Created by Odin on 2026/04/03.
//
...
final public class AdMobVideoMixAdapter: APSSPVideoMixAdAdapterProtocol {
    ...
    // 내부적으로 기존 adapter 재사용
    private var interstitialAdapter: AdMobInterstitialAdapter?
    private var interstitialVideoAdapter: AdMobInterstitialVideoAdapter?
    private var rewardVideoAdapter: AdMobRewardVideoAdapter?
    ...
    public func connectVideoMixDelegate(delegate: APSSPVideoMixAdAdapterDelegate) {
        ...
        case .Interstitial:
            interstitialAdapter = AdMobInterstitialAdapter(placementDic: placementDic, rootViewController: rootViewController, info: [:])
            interstitialAdapter?.connectDelegate(delegate: self)
            
        case .InterstitialVideo:
            interstitialVideoAdapter = AdMobInterstitialVideoAdapter(placementDic: placementDic, rootViewController: rootViewController, info: [:])
            interstitialVideoAdapter?.connectDelegate(delegate: self)
            
        case .RewardVideo:
            rewardVideoAdapter = AdMobRewardVideoAdapter(placementDic: placementDic, rootViewController: rootViewController, info: [:])
...
extension AdMobVideoMixAdapter: APSSPInterstitialAdapterDelegate {
...
extension AdMobVideoMixAdapter: APSSPInterstitialVideoAdapterDelegate {
...
extension AdMobVideoMixAdapter: APSSPRewardVideoAdapterDelegate {
    ...
    public func rewardVideoCompleted() {
        videoMixDelegate?.videoMixAdCompleteTrackingEvent(adNetworkNo: APSSPMediationCompany.AdMob.rawValue, isCompleted: true, type: videoMixAdType)
    }
}
